...
exports.getWorkingContext = async function (context) {
  const workingContext = {
    repo: await findRepoName(),
    branch: context.getWorkingBranch(),
  };

  if (!workingContext.branch && context.repos.includes(workingContext.repo)) {
    workingContext.branch = await findWorkingBranch();
  }
...
async function findRepoName() {
  ...
  return fullRepo.endsWith(GIT_EXTENSION) ? fullRepo.slice(0, -GIT_EXTENSION.length) : fullRepo;
}
