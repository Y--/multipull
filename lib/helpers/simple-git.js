...
module.exports.findPullRequestsOnBranch = async function(context, repo, currentBranch) {
  ...
  for (const [reviews, status, pr] of prsDetails) {
    res.push({
      ...
      ci_status: {
        state: status.state,
        statuses: status.statuses.map(s => pick(s, ['state', 'description', 'created_at', 'context'])),
      },
...
function __patchRepositoryPrototype(ghRepo) {
  ...
  ghRepo.constructor.prototype.getCombinedStatus = function(ref, options, cb) {
    return this._request('GET', `/repos/${this.__fullname}/commits/${ref}/status`, null, cb);
  };
