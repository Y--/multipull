...
class Processor {
  ...
  async processRepo(repo) {
    ...
    try {
      ...
      if (!err.message.includes('on a directory that does not exist')) {
        result.res = await this.context.getRepoCommonStatus(repo);
      }
    ...
    debug.enabled && debug(`Completed task ${repo} (${result.elapsed} ms)`);
    this.progress?.tick();
