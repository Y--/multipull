...
module.exports = function printResults(config, startTs, res) {
  ...
  const head  = ['', 'Current', 'Tracking', 'S', '??', 'M', 'D', 'A', 'C', 'Files', 'Changes', 'Insertions', 'Deletions', 'Pushed', 'E'];
  ...
  for (const [i, repo] of repos.entries()) {
    ...
    const { pull = defaultPull, status, stash } = res[i].res;
    ...
    const line     = [currTxt, tracking, t(stash.total), ...statuses, ...pulls, t(status.pushed)];
    if (line.find(e => e !== '')) {
