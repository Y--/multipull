...
module.exports = function printResults(context, startTs, res) {
  ...
  for (const [i, repo] of context.repos.entries()) {
    ...
    const line = [
      ...
      t(pushed),
      t(merged),
      context.toPrintableUrl(pr),
      t(reviews),
      approved,
      formatBuildUrl(context, result.buildState, buildURL),
      t(buildStatus),
      t(state),
      t(stash.total),
      ...statuses,
    ...
    if (context.config.all || isNonEmptyCollection(line)) {
...
function formatBuildUrl(context, state, url) {
  switch (state) {
  case 'failure':
    return colors.red(context.toPrintableUrl(url));
  case 'pending':
    return colors.yellow(context.toPrintableUrl(url));
  case 'success':
    return colors.green(context.toPrintableUrl(url));
  }
