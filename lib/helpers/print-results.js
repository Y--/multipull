...
module.exports = function printResults(context, startTs, res) {
  ...
  const { repos } = context;
  console.log(`Checked ${repos.length} repositorie${s(repos)} in : ${elapsed / 1000}s @ ${new Date().toString()}`);

  if (errors.length) {
    console.error(`${errors.length} error${s(errors)} occured while pulling repos :`);
  }
