...
module.exports = function printResults(context, startTs, res) {
  ...
  const head  = ['', 'Current', 'Tracking', 'Pushed', 'Merged', 'Pull Request', 'State', 'Reviews', 'Approved', 'Build Status', 'Build URL', 'S', '??', 'M', 'D', 'A', 'C', 'Files', 'Changes', 'Insertions', 'Deletions', 'Error', 'E'];
  ...
  for (const [i, repo] of context.repos.entries()) {
    ...
    const approved = result.approved ? 'Yes' : '';
    const line     = [currTxt, tracking, t(pushed), t(merged), t(pr), t(state), t(reviews), approved, t(buildStatus), t(buildURL), t(stash.total), ...statuses, ...pulls, errorCell];
