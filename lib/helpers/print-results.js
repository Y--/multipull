...
module.exports = function printResults(context, startTs, res) {
  ...
  if (context.config.pr && context.config.open) {
    openPullRequests(res);
  }

  if (!Array.isArray(res)) {
    return logger.logError(res.err);
  }

  const head  = ['', 'Current', 'Tracking', 'Pushed', 'Merged', 'Pull Request', 'State', 'Build', 'Reviews', 'S', '??', 'M', 'D', 'A', 'C', 'Files', 'Changes', 'Insertions', 'Deletions', 'Error', 'E'];
  ...
  for (const [i, repo] of context.repos.entries()) {
    ...
    const { pull = defaultPull, status, stash, pushed, merged, pr, state, build, reviews } = result;
    ...
    const line     = [currTxt, tracking, t(pushed), t(merged), t(pr), t(state), t(build), t(reviews), t(stash.total), ...statuses, ...pulls, errorCell];
    if (line.find(e => e !== '')) {
...
function openPullRequests(results) {
  for (const { res } of results) {
    if (res.pr) {
      cp.exec(`open ${res.pr}`);
    }
