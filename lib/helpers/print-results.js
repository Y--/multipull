...
module.exports = function printResults(context, startTs, res) {
  const head  = ['', 'Current', 'Tracking', 'Pushed', 'Merged', 'Pull Request', 'S', '??', 'M', 'D', 'A', 'C', 'Files', 'Changes', 'Insertions', 'Deletions', 'E'];
  ...
  for (const [i, repo] of context.repos.entries()) {
    ...
    const { pull = defaultPull, status, stash, pushed, merged, pr } = res[i].res;
    ...
    const line     = [currTxt, tracking, t(pushed), t(merged), t(pr), t(stash.total), ...statuses, ...pulls];
    if (line.find(e => e !== '')) {
