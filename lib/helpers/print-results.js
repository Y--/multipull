...
function highlightSubrepos(table) {
  const prefixStack = [];
  for (const entry of table) {
    ...
    while (prefixStack.length > 0) {
      ...
      entry['  '.repeat(prefixStack.length + 1) + repoName.slice(last.length + 1)] = entry[repoName];
      delete entry[repoName];
      prefixStack.push(last);
      break;
    }

    prefixStack.push(repoName);
  }
