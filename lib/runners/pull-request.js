...
module.exports = [{
  ...
  async runner(context, checkoutResults) {
    ...
    const reviewers = findForcedReviewers(context);
    const countRepos = candidates.length === 1 ? '' : `${candidates.length} repo${s(candidates)}, `;
    const reposStr = countRepos + formatArr(candidates);
    const reviewersStr = reviewers ? formatArr(reviewers) + ' as reviewer' + s(reviewers) : 'no reviwer';
    const valid = await utils.getYNAnswer(`Do you want to create a PR in ${reposStr} with ${reviewersStr}?`);
    if (!valid) {
  ...
  async runner(context, repo) {
    ...
    const reviewers = context.reviewers || findRandomReviewers(context);
    if (!reviewers) {
      return;
    }
