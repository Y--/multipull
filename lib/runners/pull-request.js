...
module.exports = [
  {
    ...
    async runner(context) {
      const { repo, branch } = await utils.getWorkingContext(context);
      if (!branch) {
      ...
      if (branch === defaultBranch) {
        throw new Error(`Refusing to create a PR on '${defaultBranch}'`);
      }

      context.setWorkingBranch(branch);
      logger.logInfo(`Will work with pull requests on ${colors.bold(branch)}.`);
    },
  },
  {
    ...
    async runner(context, repo) {
      ...
      {
        const { data } = await ghRepo.getReviews(pr.number);
        pr.reviews = data;
      }
...
async function createPR(context, repo, pr) {
  ...
  const ghRepo = await context.getGitHubAPI(repo);
  const params = getPullRequestCreationParams(context, repo);
...
function getPullRequestCreationParams(context, repo) {
  const base = context.getDefaultBranch(repo);
  const params = { title: context.defaultTitle, body: '', head: context.getWorkingBranch(), base };
...
async function createReviewRequest(ghRepo, prResult, reviewers, repo) {
  ...
  const teamReviewers = [];
  const individualReviewers = [];
  for (const reviewer of reviewers) {
    if (reviewer.startsWith('team/')) {
      teamReviewers.push(reviewer.slice('team/'.length));
    } else {
      individualReviewers.push(reviewer);
    }
  }
  try {
    await ghRepo.createReviewRequest(prResult.number, { reviewers: individualReviewers, team_reviewers: teamReviewers });
  } catch (err) {
...
function computeDefaultDescription(context, prPerRepo) {
  return {
    title: computeDefaultPRTitle(context.getWorkingBranch(), prPerRepo.keys()),
    body: computeDefaultPRBody(context, prPerRepo),
  };
}
...
function computeDefaultPRBody(context, prPerRepo) {
  const repoSuffix = prPerRepo.size > 1 ? 'ies' : 'y';
  const parts = [`Pull request in ${prPerRepo.size} repositor${repoSuffix}:`];
  for (const [repo, pr] of prPerRepo) {
    const lineUrl = pr ? `: [${pr.html_url}](${pr.html_url})` : 'url pending';
    parts.push(`* \`${repo}\` ${lineUrl}`);
  }
...
function findApplicableUpdatesAlongCreations(context, candidates) {
  ...
  const prPerRepo = new Map();
  for (const candidate of candidates) {
    prPerRepo.set(candidate.repo, candidate.res.pr);
  }

  const forcedReviewers = findForcedReviewers(context);
  const defaultDescription = computeDefaultDescription(context, prPerRepo);
