...
module.exports = async function doPushRepo(context, repo) {
  ...
  const pushed = await pushRepo(sg, context, pushResult.status);

  return Object.assign(pushResult, { pushed });
};

async function pushRepo(sg, context, status) {
  const { force } = context.args;
  const { ahead, behind, current, tracking } = status;
  if (behind !== 0 && !force) {
    return `${current} is behind ${behind} commit${s(behind)} from ${tracking}`;
  }
