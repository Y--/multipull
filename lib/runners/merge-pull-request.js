...
module.exports = [
  {
    ...
    async runner(context) {
      ...
      if (!branch || branch === context.getDefaultBranch(repo)) {
  ...
  },
  {
    ...
    async runner(context, repo) {
      ...
      const { data: prs } = await ghRepo.listPullRequests({ state: 'open', head, base });
      if (!prs.length) {
        return;
      }
  ...
  },
  {
    ...
    async runner(context, repo) {
      ...
      const ref = pr.head.label.includes(pr.head.ref) ? '' : '/' + pr.head.ref;
      const { data } = await ghRepo.mergePullRequest(pr.number, {
        commit_title: `Merge pull request #${pr.number} from ${pr.head.label}${ref}`,
        commit_message: '',
        sha: pr.head.sha,
