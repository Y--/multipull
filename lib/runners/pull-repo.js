...
async function pullRepoIfNotAhead(sg, repo) {
  ...
  if (!status.behind && !isOriginUnsetAndBehindOriginMaster(status)) {
    return { files : [], summary : {} };
  }

  const isLocalClean = isClean(status);
  if (!status.ahead && isLocalClean) {
    return rebaseRepo(sg, status, true);
  }
...
function isOriginUnsetAndBehindOriginMaster(status) {
  return status.tracking === null && status.diff_with_origin_master && status.diff_with_origin_master.behind > 0;
}
